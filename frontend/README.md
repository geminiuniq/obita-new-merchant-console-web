...
| `#payouts`    | mock     | KPI strip + withdrawal table with chain badges + 4-eyes status pills (`PENDING_APPROVAL` / `APPROVED` / `BROADCAST` / `CONFIRMED` / `REJECTED`). Backend schema already enforces 4-eyes, allowlist, 24h cool-down — controller ships P1 |
